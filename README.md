...
Change this from `Write` to `Read`.

#### Rational
All ToIP will be added to the "Members" Team.
The "Member" Team will be added to all repos with `Triage` access.
Contributions from members without `Write` access can be done via the fork/pull request workflow.
...
### Recommendation
Create a GitHub team for each existing Working Group and a child team for the WG Task Forces. Initially using the chairs for assigning GitHub `Write` access.
...
* Ecosystem Foundry Working Group (`efwg`)
    ...
    * Good Health Pass Working Group (`efwg-ghp-tf`) (anomaly it is named a working group under the task force list)
...
##### Repercussion
* `trustoverip/logo-assets` reassigned to the "owners" team
...
##### Recommendation
* Update with existing ToIP members
* Add to existing repos with `Triage` role.
...
### Archive
Repositories not active, or not under the governance of an existing WG/TF.

#### Rational

Keeping a clean and up-to-date set of repositories will make maintenance moving forward manageable.owners
